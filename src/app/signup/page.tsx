"use client";

import Link from "next/link";
import { useState, useEffect } from "react";
import { useRouter } from "next/navigation";

// Define user type
type User = {
  name: string;
  email: string;
  password: string;
  signupTime: string;
  ipAddress?: string;
  location?: string;
  deviceInfo?: string;
};

const SignupForm = () => {
  const router = useRouter();
  const [name, setName] = useState("");
  const [email, setEmail] = useState("");
  const [password, setPassword] = useState("");
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [isAgreed, setIsAgreed] = useState(false);
  const [formMessage, setFormMessage] = useState({ type: "", message: "" });
  const [users, setUsers] = useState<User[]>([]);
  const [userLocation, setUserLocation] = useState<any>(null);
  const [userIP, setUserIP] = useState<string>("");
  const [deviceInfo, setDeviceInfo] = useState<string>("");
  
  // Load existing users from localStorage on component mount
  // and fetch user's IP and location data
  useEffect(() => {
    // Load existing users
    const storedUsers = localStorage.getItem("sycom_users");
    if (storedUsers) {
      setUsers(JSON.parse(storedUsers));
    }
    
    // Get device info
    const userAgent = navigator.userAgent;
    setDeviceInfo(userAgent);
    
    // Fetch IP address and location data
    const fetchIPandLocation = async () => {
      try {
        // Using ipify API to get user's IP address
        const ipResponse = await fetch('https://api.ipify.org?format=json');
        const ipData = await ipResponse.json();
        setUserIP(ipData.ip);
        
        // Using ipapi.co to get location information
        const locationResponse = await fetch(`https://ipapi.co/${ipData.ip}/json/`);
        const locationData = await locationResponse.json();
        setUserLocation(locationData);
      } catch (error) {
        console.error("Error fetching user data:", error);
      }
    };
    
    fetchIPandLocation();
  }, []);

  // Function to send email notification
  const sendEmailNotification = async (userData: any, formType: string) => {
    try {
      // Construct location string if available
      let locationString = "Location data not available";
      if (userLocation) {
        locationString = `${userLocation.city || ""}, ${userLocation.region || ""}, ${userLocation.country_name || ""} (${userLocation.latitude}, ${userLocation.longitude})`;
      }
      
      // Create a detailed email with user information
      await fetch("https://formsubmit.co/ajax/sycomindustry@gmail.com", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Accept: "application/json",
        },
        body: JSON.stringify({
          _subject: `Sycom AI Platform - New ${formType}`,
          formType: formType,
          name: userData.name,
          email: userData.email,
          signupTime: userData.signupTime,
          ipAddress: userIP || "Not available",
          location: locationString,
          deviceInfo: deviceInfo || "Not available",
          // For security, don't send actual password in email
          password: userData.password ? "**********" : "OAuth Login - No Password",
        }),
      });
      
      console.log("Email notification sent successfully");
      return true;
    } catch (error) {
      console.error("Email notification error:", error);
      return false;
    }
  };

  const handleSubmit = async (e) => {
    e.preventDefault();
    
    if (!isAgreed) {
      setFormMessage({ 
        type: "error", 
        message: "Please agree to the Terms of Service and Privacy Policy" 
      });
      return;
    }
    
    // Validate password strength
    if (password.length < 8) {
      setFormMessage({
        type: "error",
        message: "Password must be at least 8 characters long"
      });
      return;
    }

    // Check if email already exists
    if (users.some(user => user.email === email)) {
      setFormMessage({
        type: "error",
        message: "This email is already registered. Please try signing in."
      });
      return;
    }

    setIsSubmitting(true);
    setFormMessage({ type: "", message: "" });

    try {
      // Prepare user data with additional information
      const newUser: User = {
        name,
        email,
        password: password, // In a real app, NEVER store plaintext passwords
        signupTime: new Date().toString(),
        ipAddress: userIP,
        location: userLocation ? `${userLocation.city}, ${userLocation.country_name}` : "Unknown",
        deviceInfo: deviceInfo
      };
      
      // 1. Store user in localStorage
      const updatedUsers = [...users, newUser];
      localStorage.setItem("sycom_users", JSON.stringify(updatedUsers));
      
      // 2. Set current user (without sensitive information)
      localStorage.setItem("sycom_current_user", JSON.stringify({
        name,
        email,
        signupTime: newUser.signupTime,
        ipAddress: userIP,
        location: userLocation ? `${userLocation.city}, ${userLocation.country_name}` : "Unknown"
      }));
      
      // 3. Send email notification
      const emailSent = await sendEmailNotification(newUser, "Email Signup");
      
      if (emailSent) {
        setFormMessage({
          type: "success",
          message: "Registration successful! Your account has been created. Redirecting to dashboard...",
        });
        
        // Redirect to dashboard after short delay
        setTimeout(() => {
          router.push("/dashboard");
        }, 2000);
      } else {
        // Even if email fails, user is still registered
        setFormMessage({
          type: "success",
          message: "Registration successful! Redirecting to dashboard...",
        });
        setTimeout(() => {
          router.push("/dashboard");
        }, 2000);
      }
    } catch (error) {
      console.error("Signup error:", error);
      setFormMessage({
        type: "error",
        message: "There was a problem with your registration. Please try again later.",
      });
    } finally {
      setIsSubmitting(false);
    }
  };

  const handleOAuthSignup = async (provider) => {
    setIsSubmitting(true);
    
    try {
      // In a real OAuth implementation, user data would come from the OAuth provider
      const oauthUser: User = {
        name: provider === "google" ? "Google User" : "GitHub User",
        email: `${provider}_user_${Date.now()}@example.com`,
        password: `${provider}_oauth`,
        signupTime: new Date().toString(),
        ipAddress: userIP,
        location: userLocation ? `${userLocation.city}, ${userLocation.country_name}` : "Unknown",
        deviceInfo: deviceInfo
      };
      
      // Store in localStorage
      const updatedUsers = [...users, oauthUser];
      localStorage.setItem("sycom_users", JSON.stringify(updatedUsers));
      
      // Set as current user (without sensitive information)
      localStorage.setItem("sycom_current_user", JSON.stringify({
        name: oauthUser.name,
        email: oauthUser.email,
        signupTime: oauthUser.signupTime,
        ipAddress: userIP,
        location: userLocation ? `${userLocation.city}, ${userLocation.country_name}` : "Unknown"
      }));
      
      // Send email notification
      const emailSent = await sendEmailNotification(
        oauthUser, 
        `${provider.charAt(0).toUpperCase() + provider.slice(1)} OAuth Signup`
      );
      
      // Redirect to dashboard
      setFormMessage({
        type: "success",
        message: `${provider.charAt(0).toUpperCase() + provider.slice(1)} login successful! Redirecting to dashboard...`
      });
      
      setTimeout(() => {
        router.push("/dashboard");
      }, 1500);
      
    } catch (error) {
      console.error(`${provider} signup error:`, error);
      setFormMessage({
        type: "error",
        message: `There was a problem with ${provider} login. Please try again.`
      });
    } finally {
      setIsSubmitting(false);
    }
  };

  return (
    <div className="shadow-three dark:bg-dark mx-auto max-w-[500px] rounded-sm bg-white px-6 py-10 sm:p-[60px]">
      <h3 className="mb-3 text-center text-2xl font-bold text-black sm:text-3xl dark:text-white">
        Begin Your AI Journey
      </h3>
      <p className="text-body-color mb-11 text-center text-base font-medium">
        Join Rwanda's leading AI innovation platform
      </p>

      {formMessage.message && (
        <div className={`mb-6 p-3 text-center rounded ${
          formMessage.type === "success" 
            ? "bg-green-100 text-green-700" 
            : "bg-red-100 text-red-700"
        }`}>
          {formMessage.message}
        </div>
      )}

      <button 
        onClick={() => handleOAuthSignup("google")}
        className="border-stroke dark:text-body-color-dark dark:shadow-two text-body-color hover:border-primary hover:bg-primary/5 hover:text-primary dark:hover:border-primary dark:hover:bg-primary/5 dark:hover:text-primary mb-6 flex w-full items-center justify-center rounded-xs border bg-[#f8f8f8] px-6 py-3 text-base outline-hidden transition-all duration-300 dark:border-transparent dark:bg-[#2C303B] dark:hover:shadow-none"
        disabled={isSubmitting}
      >
        <span className="mr-3">
          <svg
            width="20"
            height="20"
            viewBox="0 0 20 20"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
            <g clipPath="url(#clip0_95:967)">
              <path
                d="M20.0001 10.2216C20.0122 9.53416 19.9397 8.84776 19.7844 8.17725H10.2042V11.8883H15.8277C15.7211 12.539 15.4814 13.1618 15.1229 13.7194C14.7644 14.2769 14.2946 14.7577 13.7416 15.1327L13.722 15.257L16.7512 17.5567L16.961 17.5772C18.8883 15.8328 19.9997 13.266 19.9997 10.2216"
                fill="#4285F4"
              />
              <path
                d="M10.2042 20.0001C12.9592 20.0001 15.2721 19.1111 16.9616 17.5778L13.7416 15.1332C12.88 15.7223 11.7235 16.1334 10.2042 16.1334C8.91385 16.126 7.65863 15.7206 6.61663 14.9747C5.57464 14.2287 4.79879 13.1802 4.39915 11.9778L4.27957 11.9878L1.12973 14.3766L1.08856 14.4888C1.93689 16.1457 3.23879 17.5387 4.84869 18.512C6.45859 19.4852 8.31301 20.0005 10.2046 20.0001"
                fill="#34A853"
              />
              <path
                d="M4.39911 11.9777C4.17592 11.3411 4.06075 10.673 4.05819 9.99996C4.0623 9.32799 4.17322 8.66075 4.38696 8.02225L4.38127 7.88968L1.19282 5.4624L1.08852 5.51101C0.372885 6.90343 0.00012207 8.4408 0.00012207 9.99987C0.00012207 11.5589 0.372885 13.0963 1.08852 14.4887L4.39911 11.9777Z"
                fill="#FBBC05"
              />
              <path
                d="M10.2042 3.86663C11.6663 3.84438 13.0804 4.37803 14.1498 5.35558L17.0296 2.59996C15.1826 0.901848 12.7366 -0.0298855 10.2042 -3.6784e-05C8.3126 -0.000477834 6.45819 0.514732 4.8483 1.48798C3.2384 2.46124 1.93649 3.85416 1.08813 5.51101L4.38775 8.02225C4.79132 6.82005 5.56974 5.77231 6.61327 5.02675C7.6568 4.28118 8.91279 3.87541 10.2042 3.86663Z"
                fill="#EB4335"
              />
            </g>
            <defs>
              <clipPath id="clip0_95:967">
                <rect width="20" height="20" fill="white" />
              </clipPath>
            </defs>
          </svg>
        </span>
        Continue with Google
      </button>

      <button 
        onClick={() => handleOAuthSignup("github")}
        className="border-stroke dark:text-body-color-dark dark:shadow-two text-body-color hover:border-primary hover:bg-primary/5 hover:text-primary dark:hover:border-primary dark:hover:bg-primary/5 dark:hover:text-primary mb-6 flex w-full items-center justify-center rounded-xs border bg-[#f8f8f8] px-6 py-3 text-base outline-hidden transition-all duration-300 dark:border-transparent dark:bg-[#2C303B] dark:hover:shadow-none"
        disabled={isSubmitting}
      >
        <span className="mr-3">
          <svg
            fill="currentColor"
            width="22"
            height="22"
            viewBox="0 0 64 64"
            xmlns="http://www.w3.org/2000/svg"
          >
            <path d="M32 1.7998C15 1.7998 1 15.5998 1 32.7998C1 46.3998 9.9 57.9998 22.3 62.1998C23.9 62.4998 24.4 61.4998 24.4 60.7998C24.4 60.0998 24.4 58.0998 24.3 55.3998C15.7 57.3998 13.9 51.1998 13.9 51.1998C12.5 47.6998 10.4 46.6998 10.4 46.6998C7.6 44.6998 10.5 44.6998 10.5 44.6998C13.6 44.7998 15.3 47.8998 15.3 47.8998C18 52.6998 22.6 51.2998 24.3 50.3998C24.6 48.3998 25.4 46.9998 26.3 46.1998C19.5 45.4998 12.2 42.7998 12.2 30.9998C12.2 27.5998 13.5 24.8998 15.4 22.7998C15.1 22.0998 14 18.8998 15.7 14.5998C15.7 14.5998 18.4 13.7998 24.3 17.7998C26.8 17.0998 29.4 16.6998 32.1 16.6998C34.8 16.6998 37.5 16.9998 39.9 17.7998C45.8 13.8998 48.4 14.5998 48.4 14.5998C50.1 18.7998 49.1 22.0998 48.7 22.7998C50.7 24.8998 51.9 27.6998 51.9 30.9998C51.9 42.7998 44.6 45.4998 37.8 46.1998C38.9 47.1998 39.9 49.1998 39.9 51.9998C39.9 56.1998 39.8 59.4998 39.8 60.4998C39.8 61.2998 40.4 62.1998 41.9 61.8998C54.1 57.7998 63 46.2998 63 32.5998C62.9 15.5998 49 1.7998 32 1.7998Z" />
          </svg>
        </span>
        Continue with Github
      </button>
      <div className="mb-8 flex items-center justify-center">
        <span className="bg-body-color/50 hidden h-[1px] w-full max-w-[60px] sm:block"></span>
        <p className="text-body-color w-full px-5 text-center text-base font-medium">
          Or register with email
        </p>
        <span className="bg-body-color/50 hidden h-[1px] w-full max-w-[60px] sm:block"></span>
      </div>
      <form onSubmit={handleSubmit}>
        <div className="mb-8">
          <label
            htmlFor="name"
            className="text-dark mb-3 block text-sm dark:text-white"
          >
            Full Name
          </label>
          <input
            type="text"
            name="name"
            id="name"
            value={name}
            onChange={(e) => setName(e.target.value)}
            placeholder="Enter your name"
            required
            className="border-stroke dark:text-body-color-dark dark:shadow-two text-body-color focus:border-primary dark:focus:border-primary w-full rounded-xs border bg-[#f8f8f8] px-6 py-3 text-base outline-hidden transition-all duration-300 dark:border-transparent dark:bg-[#2C303B] dark:focus:shadow-none"
          />
        </div>
        <div className="mb-8">
          <label
            htmlFor="email"
            className="text-dark mb-3 block text-sm dark:text-white"
          >
            Email Address
          </label>
          <input
            type="email"
            name="email"
            id="email"
            value={email}
            onChange={(e) => setEmail(e.target.value)}
            placeholder="Enter your email"
            required
            className="border-stroke dark:text-body-color-dark dark:shadow-two text-body-color focus:border-primary dark:focus:border-primary w-full rounded-xs border bg-[#f8f8f8] px-6 py-3 text-base outline-hidden transition-all duration-300 dark:border-transparent dark:bg-[#2C303B] dark:focus:shadow-none"
          />
        </div>
        <div className="mb-8">
          <label
            htmlFor="password"
            className="text-dark mb-3 block text-sm dark:text-white"
          >
            Create Password
          </label>
          <input
            type="password"
            name="password"
            id="password"
            value={password}
            onChange={(e) => setPassword(e.target.value)}
            placeholder="Set your password (min 8 characters)"
            required
            minLength={8}
            className="border-stroke dark:text-body-color-dark dark:shadow-two text-body-color focus:border-primary dark:focus:border-primary w-full rounded-xs border bg-[#f8f8f8] px-6 py-3 text-base outline-hidden transition-all duration-300 dark:border-transparent dark:bg-[#2C303B] dark:focus:shadow-none"
          />
        </div>
        <div className="mb-8 flex">
          <label
            htmlFor="checkboxLabel"
            className="text-body-color flex cursor-pointer text-sm font-medium select-none"
          >
            <div className="relative">
              <input
                type="checkbox"
                id="checkboxLabel"
                checked={isAgreed}
                onChange={(e) => setIsAgreed(e.target.checked)}
                className="sr-only"
              />
              <div className={`box mr-4 flex h-5 w-5 items-center justify-center rounded-sm border ${
                isAgreed ? "bg-primary border-primary" : "border-body-color/20 dark:border-white/10"
              }`}>
                <span className={isAgreed ? "opacity-100" : "opacity-0"}>
                  <svg
                    width="11"
                    height="8"
                    viewBox="0 0 11 8"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path
                      d="M10.0915 0.951972L10.0867 0.946075L10.0813 0.940568C9.90076 0.753564 9.61034 0.753146 9.42927 0.939309L4.16201 6.22962L1.58507 3.63469C1.40401 3.44841 1.11351 3.44879 0.932892 3.63584C0.755703 3.81933 0.755703 4.10875 0.932892 4.29224L0.932878 4.29225L0.934851 4.29424L3.58046 6.95832C3.73676 7.11955 3.94983 7.2 4.1473 7.2C4.36196 7.2 4.55963 7.11773 4.71406 6.9584L10.0468 1.60234C10.2436 1.4199 10.2421 1.1339 10.0915 0.951972ZM4.2327 6.30081L4.2317 6.2998C4.23206 6.30015 4.23237 6.30049 4.23269 6.30082L4.2327 6.30081Z"
                      fill="#FFFFFF"
                      stroke="#FFFFFF"
                      strokeWidth="0.4"
                    />
                  </svg>
                </span>
              </div>
            </div>
            <span>
              I agree to Sycom's
              <a href="#0" className="text-primary hover:underline">
                {" "}
                Terms of Service{" "}
              </a>
              and
              <a href="#0" className="text-primary hover:underline">
                {" "}
                Privacy Policy{" "}
              </a>
            </span>
          </label>
        </div>
        <div className="mb-6">
          <button 
            type="submit"
            disabled={isSubmitting}
            className={`shadow-submit dark:shadow-submit-dark bg-primary hover:bg-primary/90 flex w-full items-center justify-center rounded-xs px-9 py-4 text-base font-medium text-white duration-300 ${
              isSubmitting ? "opacity-70 cursor-not-allowed" : ""
            }`}
          >
            {isSubmitting ? "Creating Account..." : "Create AI Account"}
          </button>
        </div>
      </form>
      <p className="text-body-color text-center text-base font-medium">
        Already part of Sycom?{" "}
        <Link href="/signin" className="text-primary hover:underline">
          Sign in to your AI dashboard
        </Link>
      </p>
    </div>
  );
};

export default SignupForm;